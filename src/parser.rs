...
#[derive(Debug, Deserialize)]
pub struct Member {
    #[serde(deserialize_with = "parse_json_usize")]
    pub id: usize,
...
fn parse_json_usize<'de, D>(de: D) -> Result<usize, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let value: Value = Deserialize::deserialize(de)?;
    match value {
        Value::Number(n) => Ok(n.as_u64().ok_or_else(|| D::Error::custom("overflow"))? as usize),
        Value::String(n) => Ok(n.parse().map_err(D::Error::custom)?),
        _ => Err(D::Error::custom("expected unsigned integer")),
    }
}
